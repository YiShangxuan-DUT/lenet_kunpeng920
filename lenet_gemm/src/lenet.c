//gemm
/* ----------  src/lenet.c  (im2col + 4×4 SGEMM, retains original ops) ---------- */
#include "lenet.h"
#include "utils.h"
#include "sgemm4x4.h"      /* NEW */
...
/* =============================================================
 * 2. 专用 5×5 stride=1, pad=0（C 版本，无边界检查）
 * =========================================================== */
...
/* =============================================================
 * 4. im2col 5×5, stride=1, pad=0  (for conv-2)
 * =========================================================== */
static void im2col_5x5_s1(const Tensor *in, Tensor *col)
{
    const int N  = in->n;
    const int IC = in->c;
    const int IH = in->h;
    ...
    for (int n = 0; n < N; ++n) {
        dtype *dst = col->data + (size_t)n * 150 * 64;

        for (int ic = 0; ic < IC; ++ic)
        for (int kh = 0; kh < K;  ++kh)
        for (int kw = 0; kw < K;  ++kw) {
            const dtype *in_ptr = in->data + (size_t)n * IC * IH * IW
                                            + (size_t)ic * IH * IW
                                            + (size_t)kh * IW + kw;
            for (int oh = 0; oh < OH; ++oh) {
                const dtype *row = in_ptr + oh * IW;
                memcpy(dst, row, OW * sizeof(dtype));
                dst += OW;
            }
        }
    }
...
/* =============================================================
 * 5. SGEMM wrapper conv (conv-2)
 * =========================================================== */
static void conv2d_5x5_s1_sgemm(const Tensor *im, Tensor *out,
                                const Tensor *w, const dtype *bias,
                                Tensor *col)
{
    const int N = im->n;
    const int M = w->n;
    const int K = w->c * w->h * w->w;
    const int R = 64;

    im2col_5x5_s1(im, col);  // fill (N, 64, 150)

    for (int n = 0; n < N; ++n) {
        const dtype *B = col->data  + (size_t)n * R * K;
        dtype       *C = out->data + (size_t)n * M * R;
        const dtype *A = w->data;

        memset(C, 0, sizeof(dtype) * M * R);

        sgemm4x4(M, R, K, (const float *)A, (const float *)B, (float *)C);

        for (int oc = 0; oc < M; ++oc) {
            dtype *row = C + (size_t)oc * R;
            acc_t b = (acc_t)bias[oc];
            for (int r = 0; r < R; ++r) {
                acc_t v = (acc_t)row[r] + b;
                row[r] = (dtype)(v > 0 ? v : 0);
            }
        }
    }
...
/* =============================================================
 * 6. 其余算子：池化 / ReLU / FC / Softmax  (原样保留)
 * =========================================================== */
...
#ifdef USE_NEON
#   define CONV1  conv2d_5x5_s1_neon
#else
#   define CONV1  conv2d_5x5_s1
#endif
...
#endif
    /* ------------ Conv-1 ------------ */
    CONV1(imgs,      &ws->x1, &w1, b1);        /* (N,1,32,32) → (N,6,28,28) */
    relu_ip(&ws->x1);
#ifdef PERF
    T_END(conv1);    T_START(pool1);
#endif
    avgpool2d(&ws->x1, &ws->x2, 2);            /* (N,6,28,28) → (N,6,14,14) */
#ifdef PERF
    T_END(pool1);    T_START(conv2);
#endif
    /* ------------ Conv-2 (im2col + SGEMM) ------------ */
    conv2d_5x5_s1_sgemm(&ws->x2, &ws->x3, &w2, b2, &ws->col);  /* (N,6,12,12) → (N,16,8,8) */
    /* ReLU 已在 conv2 内做了 */
#ifdef PERF
    T_END(conv2);    T_START(pool2);
#endif
    avgpool2d(&ws->x3, &ws->x4, 2);            /* (N,16,8,8) → (N,16,4,4) */
#ifdef PERF
    T_END(pool2);    T_START(fc1);
#endif
    /* 展平 16×4×4 → 256，直接 memcpy 到 ws->x5 */
